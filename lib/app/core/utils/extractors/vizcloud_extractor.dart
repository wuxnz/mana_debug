...
class VizCloudExtractor {
  Future<List<VideoSourceModel>> extractor(RawVideoSourceInfoModel sourceInfo) async {
    var embedHeaders =
    {
    ...
    var resolveUrl = "$nineAnimeConsumet?query=$embedId&action=vizcloud";
    
    var embedResponse = await http.get(Uri.parse(resolveUrl), headers: embedHeaders);
    
    var erJson = jsonDecode(embedResponse.body);
    var rawSources = erJson["data"]["media"]["sources"];
    var rawSourcesLinks = rawSources.map((e) => e["file"]).toList().cast<String>();
    ...
    for (var rawSource in rawSourcesLinks) {
      ...
      List<M3U8pass> videoInfo = await m3u8Helper.m3u8GenerationFileName(rawSource, VideoResolution.multi, sourceHeaders);
